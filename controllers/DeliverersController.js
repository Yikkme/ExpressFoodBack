...
const updateDeliverer = (async (req, res) => {
    const {id} = req.params;
    try{
        const updateDeliverer = await DeliverersModel.findByIdAndUpdate(id, req.body , { new: true });
        if (!updateDeliverer) {
...
module.exports = { getDeliverers, getDeliverer, addDeliverer, updateDeliverer, deleteDeliverer };
